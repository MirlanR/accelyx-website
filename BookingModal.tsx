...
const timezones = [
  { name: "Pacific Time (PT)", value: "US/Pacific", offset: -7 },
  { name: "Mountain Time (MT)", value: "US/Mountain", offset: -6 },
  { name: "Central Time (CT)", value: "US/Central", offset: -5 },
  { name: "Eastern Time (ET)", value: "US/Eastern", offset: -4 },
];
...
const defaultForm: FormState = {
  ...
  service: "", date: "", time: "", timezone: "US/Eastern",
};
...
export default function BookingModal({ isOpen, onClose }: BookingModalProps) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // Step 1: Send to n8n (existing workflow)
      await fetch(N8N_WEBHOOK_URL, {
        ...
        body: JSON.stringify({
          ...
          time: form.time, timezone: form.timezone,
        }),
      }).catch(() => null);
      ...
      try {
        ...
        console.log("GHL API Response:", { status: contactRes.status, data: contactData });

        if (contactRes.ok) {
          contactId = contactData.contactId || contactData.data?.id;
          console.log("✅ Contact created in GHL:", contactId);
        } else {
          console.error("❌ GHL API Error:", contactData.error, contactData.details);
        }
      } catch (err) {
        console.error("Error creating GHL contact:", err);
      }

      // Step 3: Create calendar event in GoHighLevel (if contact was created)
      if (contactId) {
        try {
          const eventRes = await fetch("/api/ghl-calendar", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              contactId,
  ...
  // Smart calendar logic: 4-day window with 4-hour same-day delay
  const now = new Date();
  const currentHour = now.getHours();
  const canBookToday = currentHour <= 14; // Before 2 PM, we have 4 hours until 6 PM

  const minDate = canBookToday ? now.toISOString().split("T")[0] : new Date(now.getTime() + 86400000).toISOString().split("T")[0];

  const maxDate = new Date();
  maxDate.setDate(maxDate.getDate() + 3); // 4-day window from today
      ...
      >
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="mb-4">
              <label className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--muted)" }}>
                <Zap size={11} />Service<span style={{ color: "#ef4444" }}>*</span>
              </label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
              ...
              <div>
                <label className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--muted)" }}>
                  <Clock size={11} />Time ({timezones.find(tz => tz.value === form.timezone)?.name.split("(")[1].slice(0, -1)})<span style={{ color: "#ef4444" }}>*</span>
                </label>
                ...
                >
                  ...
                  {timeSlots.map(t => {
                    // For same-day bookings, hide times before current time + 4 hours
                    if (form.date === minDate && canBookToday) {
